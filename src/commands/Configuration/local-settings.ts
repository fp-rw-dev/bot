...
import { LOCAL_SETTINGS } from "../../config/security.js";
// import { INFO } from "../config/colors.js";
...
export class UserSettingsCommand extends Command {
  public constructor(context: PieceContext, options: Command.Options) {
    super(context, {
      ...options,
      name: "user-settings",
      aliases: ["local-settings"],
      description: "This command is used to change settings for your user.",
    });
  }
  ...
  public async chatInputRun(interaction: CommandInteraction): Promise<void> {
    ...
    switch (interaction.options.getSubcommand(true)) {
      ...
      case "set": {
        const key = interaction.options.getString("key", true);
        if (!LOCAL_SETTINGS.includes(key)) return;
  ...
  public override async registerApplicationCommands(
    ...
    await this.prepare();
    const preparedCommandPayload = Object.keys(this.rawSchema)
      .filter((k) => LOCAL_SETTINGS.includes(k))
      .map((c) => ({
        name: `${
          ...
          c.split(".")[0] === "core"
            ? "core"
            : `${
                capitalizations[c.split(".")[0]] ??
                sentenceCase(c.split(".")[0])
              } plugin`
        })`,
        value: c,
      }));
